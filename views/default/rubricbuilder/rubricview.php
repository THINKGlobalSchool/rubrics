<?php
	 ...
	 */
		...
		if (isset($vars['entity']) && $vars['entity'] instanceof Rubric) {
			...
			$can_delete = false;
			if (($vars['user']->getGUID() == $rubric->owner_guid) || ($vars['user']->admin || $vars['user']->isAdmin())) {
				$can_delete = true;
			}
...
<div id="content_header" class="clearfloat">
	<div class="content_header_title"><h2><a href="<?php echo $url; ?>"><?php echo $vars['entity']->title . $rev_title; ?></a></div>
	<?php
	if ($vars['entity']->canEdit() && !$rev) {
		$fork_url = elgg_add_action_tokens_to_url($vars['url'] . "action/rubric/fork?rubric_guid=" . $vars['entity']->getGUID());
	?>
		<div class="content_header_options">
		<a class="action_button" href="<?php echo $fork_url; ?>"><?php echo elgg_echo('rubricbuilder:fork'); ?></a>
		<a class="action_button" href="<?php echo $vars['url']; ?>pg/rubric/edit/<?php echo $vars['entity']->getGUID() . $revision; ?>"><?php echo elgg_echo('edit'); ?></a>
		<?php
			if($can_delete){ 
				echo elgg_view('output/confirmlink',array(	
						'href' => $vars['url'] . "action/pages/delete?page=" . $vars['entity']->getGUID(),
						'text' => elgg_echo("delete"),
						'confirm' => elgg_echo("pages:delete:confirm"),
	...
	<div id="rubric_page" class="clearfloat">
	<div class="rubric_options clearfloat">
	<?php
		// Allow plugins to extend
		echo elgg_view("rubric/options",array('entity' => $vars['entity']));
		echo elgg_view_likes($vars['entity']);
	?>
	</div>
	<?php	
		if ($vars['entity']){
...
?>
			<!-- display tags -->
			<p class="tags">
				<?php echo elgg_view('output/tags', array('tags' => $tags));?>
			</p>
<?php
		}
	}
